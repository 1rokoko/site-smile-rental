...
interface ScooterCardGridCnProps {
...
const formatPricing = (pricing: ScooterModel['pricing']): string[] => {
  ...
  if (pricing['1-3'] !== undefined) {
    lines.push(`1-3 天 - ${pricing['1-3']} ฿/天`);
  }
  if (pricing['3-6'] !== undefined) {
    lines.push(`3-6 天 - ${pricing['3-6']} ฿/天`);
  }
  if (pricing['7-12'] !== undefined) {
    lines.push(`7-12 天 - ${pricing['7-12']} ฿/天`);
  }
  if (pricing['13-22'] !== undefined) {
    lines.push(`13-22 天 - ${pricing['13-22']} ฿/天`);
  }
  if (pricing['30'] !== undefined) {
    lines.push(`30 天 - ${pricing['30']} ฿`);
  }
  return lines;
};
...
const translateFeature = (feature: string): string => {
  if (feature.startsWith('Year')) {
    return feature.replace('Year', '年份');
  }
  ...
  if (feature.startsWith('Comfort')) {
    return feature.replace('Comfort', '舒适度');
  }
  if (feature.startsWith('Safety')) {
    return feature.replace('Safety', '安全性');
  }
  return feature;
};

export const ScooterCardGridCn: React.FC<ScooterCardGridCnProps> = ({ scooter, delay = 0 }) => {
        ...
        <div className="p-3 sm:p-4">
          <div className="mb-3">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs sm:text-sm text-gray-600">({scooter.reviewCount} 条评价)</span>
            </div>
          </div>
          ...
          <div className="mt-auto">
            ...
            >
              <span style={{ color: 'white' }}>全部照片</span>
            </button>
          </div>
        </div>
      </div>
