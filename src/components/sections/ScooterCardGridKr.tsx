...
interface ScooterCardGridKrProps {
...
const formatPricing = (pricing: ScooterModel['pricing']): string[] => {
  ...
  if (pricing['1-3'] !== undefined) {
    lines.push(`1-3일 - ${pricing['1-3']} ฿/일`);
  }
  if (pricing['3-6'] !== undefined) {
    lines.push(`3-6일 - ${pricing['3-6']} ฿/일`);
  }
  if (pricing['7-12'] !== undefined) {
    lines.push(`7-12일 - ${pricing['7-12']} ฿/일`);
  }
  if (pricing['13-22'] !== undefined) {
    lines.push(`13-22일 - ${pricing['13-22']} ฿/일`);
  }
  if (pricing['30'] !== undefined) {
    lines.push(`30일 - ${pricing['30']} ฿`);
  }
  return lines;
};
...
const translateFeature = (feature: string): string => {
  if (feature.startsWith('Year')) {
    return feature.replace('Year', '연식');
  }
  ...
  if (feature.startsWith('Comfort')) {
    return feature.replace('Comfort', '편안함');
  }
  if (feature.startsWith('Safety')) {
    return feature.replace('Safety', '안전성');
  }
  return feature;
};

export const ScooterCardGridKr: React.FC<ScooterCardGridKrProps> = ({ scooter, delay = 0 }) => {
        ...
        <div className="p-3 sm:p-4">
          <div className="mb-3">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs sm:text-sm text-gray-600">(리뷰 {scooter.reviewCount}개)</span>
            </div>
          </div>
          ...
          <div className="mt-auto">
            ...
            >
              <span style={{ color: 'white' }}>전체 사진</span>
            </button>
          </div>
        </div>
      </div>
