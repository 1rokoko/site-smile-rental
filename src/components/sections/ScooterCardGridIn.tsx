...
interface ScooterCardGridInProps {
...
const formatPricing = (pricing: ScooterModel['pricing']): string[] => {
  ...
  if (pricing['1-3'] !== undefined) {
    lines.push(`1-3 दिन - ${pricing['1-3']} ฿/दिन`);
  }
  if (pricing['3-6'] !== undefined) {
    lines.push(`3-6 दिन - ${pricing['3-6']} ฿/दिन`);
  }
  if (pricing['7-12'] !== undefined) {
    lines.push(`7-12 दिन - ${pricing['7-12']} ฿/दिन`);
  }
  if (pricing['13-22'] !== undefined) {
    lines.push(`13-22 दिन - ${pricing['13-22']} ฿/दिन`);
  }
  if (pricing['30'] !== undefined) {
    lines.push(`30 दिन - ${pricing['30']} ฿`);
  }
  return lines;
};
...
const translateFeature = (feature: string): string => {
  if (feature.startsWith('Year')) {
    return feature.replace('Year', 'वर्ष');
  }
  if (feature.startsWith('Power CC')) {
    const value = feature.replace('Power CC', '').trim();
    return `शक्ति ${value} CC`;
  }
  if (feature.startsWith('Comfort')) {
    return feature.replace('Comfort', 'आराम');
  }
  if (feature.startsWith('Safety')) {
    return feature.replace('Safety', 'सुरक्षा');
  }
  return feature;
};

export const ScooterCardGridIn: React.FC<ScooterCardGridInProps> = ({ scooter, delay = 0 }) => {
        ...
        <div className="p-3 sm:p-4">
          <div className="mb-3">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs sm:text-sm text-gray-600">({scooter.reviewCount} समीक्षाएँ)</span>
            </div>
          </div>
          ...
          <div className="mt-auto">
            ...
            >
              <span style={{ color: 'white' }}>सभी फ़ोटो</span>
            </button>
          </div>
        </div>
      </div>
