...
export const OwnerTestimonialIn: React.FC = () => {
  const { ownerQuote } = EXACT_CONTENT_IN;

  return (
    <Section padding="xl" background="surface">
      <Container size="md">
        <FadeInView animation="scaleIn">
          <AnimatedCard variant="elevated" padding="lg" className="text-center">
            <div className="flex flex-col items-center space-y-6">
              ...
              <div className="flex flex-col items-center space-y-4">
                ...
                <div>
                  <H3 className="text-text-primary mb-1">{ownerQuote.author}</H3>
                  <Body className="text-text-secondary capitalize">{ownerQuote.title}</Body>
                </div>
              </div>
            </div>
          </AnimatedCard>
