...
export const HeroMy: React.FC = () => {
  const { hero, features } = EXACT_CONTENT_MY;
  const [headlinePrefix, headlineSuffixRaw] = hero.mainHeadline.split(' untuk ');
  const [pricePrefixRaw, priceBonusRaw] = hero.priceText.split('+');

  const headlineSuffix = headlineSuffixRaw ?? '';
    ...
    <Section padding="md" background="surface">
      <Container>
        <div className="text-center max-w-4xl mx-auto transform-gpu">
          <H1 className="mb-6">
            <span className="text-black">{headlinePrefix}{headlineSuffix ? ' untuk ' : ''}</span>
            {headlineSuffix && <span className="text-orange-500">{headlineSuffix}</span>}
          </H1>
                ...
                <div className="text-left">
                  <Body className="text-black font-medium">
                    ...
                    <a
                      ...
                      className="text-orange-500 hover:text-orange-600 underline cursor-pointer"
                    >
                      Penipuan Jalan Raya Phuket: Panduan Bertahan
                    </a>
                  </Body>
                </div>
              </div>
