...
Write-Host "🖼️ Downloading REAL scooter images from original URLs..." -ForegroundColor Green

# Create directories
$scooterDir = "public/images/scooters"
$ownerDir = "public/images/owner"
$featuresDir = "public/images/features"
$bonusDir = "public/images/bonus"
$scamDir = "public/images/scam"

@($scooterDir, $ownerDir, $featuresDir, $bonusDir, $scamDir) | ForEach-Object {
    if (!(Test-Path $_)) {
...
# Original image URLs from git history (commit 4c1fa44)
$realImageMap = @{
    # Scooter images - REAL PHOTOS
...
function Download-RealImage {
    param($url, $path)
    try {
        Write-Host "📥 Downloading: $(Split-Path $path -Leaf)" -ForegroundColor Cyan
        
        # Use alternative download method to bypass potential blocks
        ...
        if (Test-Path $path) {
            ...
            if ($fileSize -gt 1000) {  # Check if file is larger than 1KB
                Write-Host "✅ Downloaded: $(Split-Path $path -Leaf) ($fileSize bytes)" -ForegroundColor Green
                return $true
            } else {
                Write-Host "⚠️ File too small, might be blocked: $(Split-Path $path -Leaf)" -ForegroundColor Yellow
...
Write-Host "`n🚀 Starting download of $totalCount REAL scooter images..." -ForegroundColor Yellow
Write-Host "⚠️ Note: CDN might be blocked, but we'll try alternative methods" -ForegroundColor Yellow

foreach ($mapping in $realImageMap.GetEnumerator()) {
    if (Download-RealImage -url $mapping.Key -path $mapping.Value) {
        $successCount++
    }
    Start-Sleep -Milliseconds 200  # Small delay between downloads
}
...
if ($successCount -lt $totalCount) {
    ...
    Write-Host "1. Use VPN to bypass CDN blocks" -ForegroundColor White
    Write-Host "2. Download manually from Google Photos links" -ForegroundColor White
    Write-Host "3. Use web scraping tools" -ForegroundColor White
}

Write-Host "`n🎯 Next steps:" -ForegroundColor Yellow
Write-Host "1. Update image-catalog.ts to use JPG files instead of SVG" -ForegroundColor White
...
Write-Host "`n✨ Real image download process completed!" -ForegroundColor Green
