...
$pm2Status = echo "[REMOVED]" | ssh root@38.180.122.239 "pm2 list"
...
$localTest = echo "[REMOVED]" | ssh root@38.180.122.239 "curl -s -o /dev/null -w '%{http_code}' http://localhost:3000"
Write-Host "Local App HTTP Code: $localTest" -ForegroundColor White

# Step 3: Test Nginx
Write-Host "Step 3: Testing Nginx..." -ForegroundColor Yellow
$nginxTest = echo "[REMOVED]" | ssh root@38.180.122.239 "curl -s -o /dev/null -w '%{http_code}' http://localhost"
Write-Host "Nginx HTTP Code: $nginxTest" -ForegroundColor White

# Step 4: Check firewall
Write-Host "Step 4: Checking firewall..." -ForegroundColor Yellow
$firewallStatus = echo "[REMOVED]" | ssh root@38.180.122.239 "ufw status"
